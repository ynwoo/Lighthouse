...
public interface StudyTagRepository extends JpaRepository<StudyTag, Integer> {
    ...
    @Query("select st from StudyTag st where st.studyId = :studyId and st.tagId = :userId and st.isValid = 1")
    Optional<Bookmark> find(@Param("studyId") int studyId, @Param("tagId") int tagId);
}
