...
public interface StudyRepository extends JpaRepository<Study, Long>, StudyRepositoryCustom {
    // 상세 정보 조회 (상세정보)
    @EntityGraph(attributePaths = {"original", "sido", "gugun", "badge", "studyTags", "studyEvals", "studyNotices", "sessions", "qnas", "participations"})
    Optional<Study> findDetailById(@Param("studyId") Long studyId);

    // 상세 정보 조회 (복제)
    @EntityGraph(attributePaths = {"sido", "gugun", "badge", "studyTags", "studyNotices", "sessions"})
    ...
    // 이전 스터디 목록 (좋아요 순)
    @Query("select new com.ssafy.lighthouse.domain.study.dto.SimpleStudyDto(s) from Study s where s.original.id = :originalId and s.isValid = 1 and s.status = 3 order by s.likeCnt desc ")
